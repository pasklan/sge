...
def get_sales_metrics():
    ...
    total_cost_price = sum(outflow.quantity * outflow.product.cost_price for outflow in Outflow.objects.all())
    total_sales_profit = total_sales_value - total_cost_price

    return dict(
        total_sales=total_sales,
        total_products_sold=total_products_sold,
        sales_value=number_format(total_sales_value, decimal_pos=2, force_grouping=True),
